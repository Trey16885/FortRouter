...
const { ResearchIndex } = require('./research');
...
class Registry {
  ...
  generate(modelId, prompt, options = {}) {
    ...
    // Research-capable models condition generation on what they retrieved.
    const context = research.length
      ? research.map((r) => r.excerpt).join(' ') + ' ' + prompt
      : prompt;
    const raw = model.generate(context, {
    ...
    const result = { model: card.id, type: 'text', prompt, text: screened.text };
    if (!screened.allowed) result.safety = { outputFiltered: true, category: screened.category };
    if (cfg.research > 0) {
      result.thinking = research.length
        ? `Searched the local index for "${prompt.slice(0, 80)}" and weighed ${research.length} relevant passage(s) before answering.`
        : 'Searched the local index but found no relevant passages; answering from base training alone.';
